...
export class Search {
  ...
  search(query: string) {
    ...
    // console.log(this.loadingSpinner.classList);
    ...
    // console.debug("results no: ", results.length);
    ...
    if (results.length > 0) {
      // console.time("poplateResults");
      this.poplateResults();
      // console.timeEnd("poplateResults");
    } else {
  ...
  highlight(id, titleKeywords, contentKeywords) {
    const titleHighlighter = new Mark(document.querySelector(`#${id} .search-result-title`));
    titleHighlighter.mark(titleKeywords, this.highlightOptions);
    const contentHighlighter = new Mark(document.querySelector(`#${id} .search-result-content`));
